...
# and : https://github.com/y0ast/DUE/blob/f29c990811fd6a8e76215f17049e6952ef5ea0c9/due/sngp.py#L52
...
from models.classification.spectralresnet import SpectralResNet18, SpectralResNet82, SpectralResNet101, \
    SpectralResNet152, SpectralResNet34
from data.datasets.shared.utils import DatasetStructure
from config import BaseConfig


def sngp_model(cfg: BaseConfig, data_cfg: DatasetStructure, extractor: str = 'resnet-18', num_classes: int = 10):
    if extractor == 'resnet-18':
        print(f'Using resnet-18 feature extractor')
        expansion = 1
        feature_extractor = SpectralResNet18()
    elif extractor == 'resnet-34':
        print(f'Using resnet-34 feature extractor')
        expansion = 1
        feature_extractor = SpectralResNet34()
    elif extractor == 'resnet-82':
        print(f'Using resnet-82 feature extractor')
        expansion = 1
        feature_extractor = SpectralResNet82()
    elif extractor == 'resnet-101':
        print(f'Using resnet-101 feature extractor')
        expansion = 4
        feature_extractor = SpectralResNet101()
    elif extractor == 'resnet-152':
        print(f'Using resnet-152 feature extractor')
        expansion = 4
        feature_extractor = SpectralResNet152()
    else:
        raise ValueError(f'Feature extractor {extractor} not implemented yet!')
    # Defaults from SNGP on CIFAR-10 in uncertainty-baselines
    num_deep_features = 512 * expansion
    num_gp_features = 128

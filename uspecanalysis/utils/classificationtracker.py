...
def get_uspec_inputs(cfg: BaseConfig):
    ...
    if cfg.data.dataset in ['CIFAR100', 'MNIST', 'SVHN'] and cfg.data.create_validation:
        name = 'val'
        loader = get_dataset(cfg=cfg)
        shape = (loader.data_config.val_len,)
        uspec_tracker = USPECTracker(shape, num_seeds=cfg.uspec_configs.num_seeds)
        fe_tracker = ClassifcationTracker(loader.data_config.val_len)
        loader_struct = (loader, fe_tracker)
        uspec_inputs.append((uspec_tracker, name, loader_struct))
    ...
    if cfg.run_configs.ood.cifar10C:
        ...
        for loader in loaders:
            ...
            loader_struct = (loader, fe_tracker)
            uspec_inputs.append((uspec_tracker, name, loader_struct))
    if cfg.run_configs.ood.cifar100C:
        name = 'CIFAR100C'
        loaders = get_dataset(cfg=cfg, override=name)
        for loader in loaders:
            name = loader.name
            ...
            loader_struct = (loader, fe_tracker)
            uspec_inputs.append((uspec_tracker, name, loader_struct))
    if cfg.run_configs.ood.medMNIST:
        datasets = ['medmnist-organA', 'medmnist-organC', 'medmnist-organS', 'medmnist-blood',
                    'medmnist-tissue', 'medmnist-pneumonia', 'medmnist-oct', 'medmnist-path',
                    'medmnist-derma']
        for ds in datasets:
            loader = get_dataset(cfg, override=ds)
            shape = (loader.data_config.test_len,)
            uspec_tracker = USPECTracker(shape, num_seeds=cfg.uspec_configs.num_seeds)
            fe_tracker = ClassifcationTracker(loader.data_config.test_len)
            loader_struct = (loader, fe_tracker)
            uspec_inputs.append((uspec_tracker, ds, loader_struct))

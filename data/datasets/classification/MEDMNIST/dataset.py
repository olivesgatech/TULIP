...
class LoaderMEDMNIST(Dataset):
    def __init__(self, data_config: DatasetStructure, split: str, current_idxs: list = None, transform=None):
        ...
        if split == 'train':
            ...
            self.X = data_config.val_set
            self.Y = data_config.val_labels
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            x = Image.fromarray(x.numpy(), mode='L') if len(x.shape) == 2 else Image.fromarray(x.numpy())
            x = self.transform(x)

...
class ClassifcationTracker(StatTracker):
    ...
    def save_statistics(self, directory: str, ld_type: str, extenstion: str = ''):
        path = directory + '/ld_statistics/' + ld_type
        if not os.path.exists(path):
            os.makedirs(path)
        if self._tracking_type in ['FE', 'all']:
            np.save(path + f'/forgetting_events{extenstion}.npy', self._forgetting_events)
        if self._tracking_type in ['SE', 'all']:
            np.save(path + f'/switch_events{extenstion}.npy', self._switch_events)

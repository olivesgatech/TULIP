...
def get_cifar100c(cfg: BaseConfig):
    path = cfg.data.data_loc
    print(os.path.expanduser(path) + '/CIFAR-100-C/')
    output_loader_list = []

    for corruption in CORRUPTIONS:
        for level in LEVELS:
            x_corrupted_te = np.load(os.path.expanduser(path) + '/CIFAR-100-C/' + corruption + '.npy')
            y_corrupted_te = np.load(os.path.expanduser(path) + '/CIFAR-100-C/labels.npy')
            ...
            loaders = LoaderObject(test_loader=test_loader,
                                   ...
                                   name= 'CIFAR100C' + corruption + str(level))

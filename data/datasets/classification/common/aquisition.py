...
from data.datasets.classification.CIFAR100C.configuration import get_cifar100c
from data.datasets.classification.CINIC10.configuration import get_cinic10
from data.datasets.classification.MEDMNIST.configuration import get_med_mnist_loaders
...
def get_dataset(cfg: BaseConfig, override: str = None, idxs: np.ndarray = None, test_bs: bool = False):
    ...
    if dataset == 'MNIST':
        ...
        return get_cifar100(cfg, idxs, num_classes=cfg.data.num_classes)
        ...
        if not override:
            raise Exception('CIFAR10C can not be trained on as it is a corruption dataset!')
        return get_cifar100c(cfg)
    elif dataset.split('-')[0] == 'medmnist':
        return get_med_mnist_loaders(cfg, dataset, idxs)
    else:
